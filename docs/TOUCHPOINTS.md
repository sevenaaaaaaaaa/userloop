...
| **P2（千人千面已完成）** | 六维千面：来源 / 旅程阶段 / Cookie 状态 / UTM / 设备 / 互动时长 | ✅ 实测：同链接移动/桌面 SSR 不同、微信/广告/邮件来源各出专属文案、阶段参数出「为你准备」、倒计时区块在场、内置页 20s 后强化 CTA |
...
| 维度 | 机制 | 说明 |
|---|---|---|
| 设备 | **SSR `audience`** 双版区块 | 移动/桌面两套主视觉互斥显示（实测 SSR 输出不同） |
| 来源 / UTM | **`personalize` 运行时补丁** | `utm_source=wechat`→「微信专享」；`utm_medium=cpc`→「限时活动」；`utm_medium=email`→「邮件专享」 |
| Cookie 状态 | `personalize`（`visitor/return`、`login`） | 回访访客→「欢迎回来」；已登录→「已登录」 |
| 旅程阶段 | 交付时写入 + 共享链接 `utm_content=stage-<stage>` | 每用户按其实时阶段；共享链接按参数命中 `seg_stage_*` |
| 互动时长 | 内置页 JS（默认 20s 强化 CTA）+ WebsFlow `countdown` 区块 | 停留越久行动越强；倒计时表达紧迫感 |

**链接模型**：campaign 模式（默认）—— 一个 campaign（模板/环节）一条链接，所有用户复用 → 真正「同一链接、千人千面」，
且避免 WebsFlow 免费版发布/项目额度耗尽；额度不足时自动清理自有旧项目/下架旧页后重试。
内置页（兜底）同样支持六维：服务端按 UA/UTM/Cookie/阶段判定，前端执行互动时长策略。
